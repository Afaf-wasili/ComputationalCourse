...
if not os.path.isfile(file_path):
    ...
    def gaussian_confidence_interval(counts, confidence=0.95):
        ...
        for i, count in enumerate(counts):
            if count > 0:
                sigma = np.sqrt(count)
                lower_bound[i] = count - z * sigma
                upper_bound[i] = count + z * sigma
            else:
                lower_bound[i] = 0
                upper_bound[i] = 0
    ...
    data_lower, data_upper = gaussian_confidence_interval(data_values, confidence=0.95)
    ...
    plt.legend()
    plt.grid(True)
#    plt.savefig('data_mc_ratio_with_errors.png')
    plt.close()
