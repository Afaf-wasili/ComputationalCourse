...
results = pool.starmap(compute_result, [(i, j, theta, phi) for i in range(theta.shape[0]) for j in range(theta.shape[1])])
pool.close()
pool.join()

# Reshape results to match theta and phi
result = np.array(results).reshape(theta.shape)

# Plotting
fig = plt.figure()
ax = fig.add_subplot(111, projection='3d')
ax.plot_surface(theta, phi, result.real, cmap='viridis')
